...
for i in range(N+1):
    ...
    # Setting up Hybridized continuous interior penalty method
    ...
    if abs(gfu1.vec[numberOfElements] - obstaclePosition) < Tol or gfu1.vec[numberOfElements] > obstaclePosition:
        ...
        a = BilinearForm(
            ...
            + alpha**2*(
                ...
                - hessenn(u) * jumpdn(v,vhat) * ds(definedon='right') \
                - hessenn(v) * jumpdn(u,uhat) * ds(definedon='right')  \
                + 3*order*order/h * jumpdn(u,uhat) * jumpdn(v,vhat) * ds(definedon='right') 
            ) 
